...
public class ExcelImport implements Closeable {
    private static final Logger logger= LoggerUtil.getLogger(ExcelImport.class);
    ...
    public <E> List<E> getDataList(Class<E> cls, MethodCallback exceptionCallback, String... groups) throws InstantiationException, IllegalAccessException {
        ...
        for (Field f : fs) {
            ...
            if (efs != null && efs.value() != null) {
                for (ExcelField ef : efs.value()) {
                    ExcelAnnotationUtils.addAnnotation(annotationList, ef, f, ExcelField.Type.IMPORT, groups);
                }
            }
        ...
        for (Method m : ms) {
            ...
            if (efs != null && efs.value() != null) {
                for (ExcelField ef : efs.value()) {
                    ExcelAnnotationUtils.addAnnotation(annotationList, ef, m, ExcelField.Type.IMPORT, groups);
                }
            }
